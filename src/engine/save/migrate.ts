...
import { CHUNK_AREA, CHUNK_SIZE, WorldType } from '../constants/world';
...
export function migrateLegacySave(save: LegacyWorldSave): WorldSave {
  ...
  const legacyChunkVolume = CHUNK_AREA * LEGACY_WORLD_SIZE_Y;
  const emptyMeta = rleEncode(new Uint8Array(legacyChunkVolume));
  for (let cz = 0; cz < chunksZ; cz++) {
    for (let cx = 0; cx < chunksX; cx++) {
      const blocks = new Uint8Array(legacyChunkVolume);
      for (let y = 0; y < LEGACY_WORLD_SIZE_Y; y++) {
