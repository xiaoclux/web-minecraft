...
describe('LightEngine', () => {
  ...
  it('火把在地下发出方块光并随距离衰减', () => {
    const { world, engine } = flatWorld(10);
    for (let x = -10; x < 10; x++) {
      world.setBlockRaw(x, 5, 5, BlockId.AIR);
    }
    world.setBlockRaw(0, 5, 5, BlockId.TORCH);
    engine.updateArea(-10, 10, 5, 5);
  ...
  it('后加载的 chunk 会把光传入相邻已点亮 chunk 的洞穴', () => {
    ...
    for (let x = 20; x < 32; x++) {
      world.setBlockRaw(x, 5, 0, BlockId.AIR);
    }
    engine.updateArea(20, 31, 0, 0);
